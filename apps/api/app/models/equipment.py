...
class Equipment(BaseModel):
    ...
    next_inspection_date: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class EquipmentInspection(BaseModel):
    ...
    result: Mapped[InspectionResult] = mapped_column(
        String(20), nullable=False, default=InspectionResult.PASS
    )
    ...
    equipment: Mapped[Equipment] = relationship(
        "Equipment", back_populates="inspections", lazy="selectin"
    )
