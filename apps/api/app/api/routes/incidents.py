...
async def list_incidents(
    ...
    base = select(Incident).where(  # noqa: E712
    ...
    result = await db.execute(
        base.order_by(Incident.occurred_at.desc()).offset(pagination.offset).limit(pagination.page_size)
    )
...
async def create_incident(body: IncidentCreate, db: DBSession, current_user: CurrentUser) -> IncidentResponse:
    ...
    classify_incident_severity(body.model_dump())
    incident = Incident(
        ...
        injuries_count=body.injuries_count,
        fatalities_count=body.fatalities_count,
...
async def update_incident(
    incident_id: UUID, body: IncidentUpdate, db: DBSession, manager: ManagerUser
) -> IncidentResponse:
    result = await db.execute(
        select(Incident).where(Incident.id == incident_id, Incident.organization_id == manager.organization_id)
    )
    inc = result.scalar_one_or_none()
    if not inc:
        raise HTTPException(status_code=404, detail="Incident not found")
...
async def assign_investigator(
    incident_id: UUID, investigator_id: UUID, db: DBSession, manager: ManagerUser
) -> IncidentResponse:
    result = await db.execute(
        select(Incident).where(Incident.id == incident_id, Incident.organization_id == manager.organization_id)
    )
...
async def submit_investigation(
    incident_id: UUID, body: IncidentUpdate, db: DBSession, current_user: CurrentUser
) -> IncidentResponse:
    result = await db.execute(
        select(Incident).where(Incident.id == incident_id, Incident.organization_id == current_user.organization_id)
    )
...
async def download_report(incident_id: UUID, db: DBSession, current_user: CurrentUser) -> Response:
    ...
    filename = f"incident-{inc.reference_number}.pdf"
    return Response(
        content=pdf_bytes,
...
async def add_witness(
    incident_id: UUID, body: WitnessCreate, db: DBSession, current_user: CurrentUser
) -> WitnessResponse:
...
async def add_attachment(
    incident_id: UUID, body: AttachmentCreate, db: DBSession, current_user: CurrentUser
) -> AttachmentResponse:
...
async def add_timeline_event(
    incident_id: UUID, body: TimelineEventCreate, db: DBSession, current_user: CurrentUser
) -> dict:
...
async def list_corrective_actions(
    ...
    result = await db.execute(
        select(CorrectiveAction).where(
            CorrectiveAction.incident_id == incident_id,
            CorrectiveAction.organization_id == current_user.organization_id,
            CorrectiveAction.is_deleted == False,  # noqa: E712
        )
    )
...
async def create_corrective_action(
    incident_id: UUID, body: CorrectiveActionCreate, db: DBSession, manager: ManagerUser
) -> CorrectiveActionResponse:
...
async def update_corrective_action(
    action_id: UUID, body: CorrectiveActionUpdate, db: DBSession, current_user: CurrentUser
) -> CorrectiveActionResponse:
    result = await db.execute(
        select(CorrectiveAction).where(
